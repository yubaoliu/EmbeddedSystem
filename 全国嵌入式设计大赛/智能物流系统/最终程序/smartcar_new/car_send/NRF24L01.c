...
#define  KEY_PING                GPIO_PIN_5  //back
#define  KEY_START                GPIO_PIN_0
#define  KEY_STOP               GPIO_PIN_1
...
void KeyInit(void)
{
    ...
    GPIOPinTypeIn(KEY_PORTG, KEY_START);
    GPIOPinTypeIn(KEY_PORTG, KEY_STOP); 
    GPIOPinTypeIn(KEY_PORTG, KEY_LEFT); 
    GPIOPinTypeIn(KEY_PORTG, KEY_STOP); 
    GPIOPinTypeIn(KEY_PORTG, KEY_RIGHT); 

    GPIOIntTypeSet(KEY_PORTG, KEY_PING,GPIO_LOW_LEVEL);      //  设置KEY2管脚的中断类型
    GPIOIntTypeSet(KEY_PORTG, KEY_START,GPIO_LOW_LEVEL);
    GPIOIntTypeSet(KEY_PORTG, KEY_STOP,GPIO_LOW_LEVEL);
    GPIOIntTypeSet(KEY_PORTG, KEY_LEFT,GPIO_LOW_LEVEL);
    GPIOIntTypeSet(KEY_PORTG, KEY_RIGHT,GPIO_LOW_LEVEL);
   

    GPIOPinIntEnable(KEY_PORTG, KEY_PING);                    //  使能KEY所在管脚的中断
    GPIOPinIntEnable(KEY_PORTG, KEY_START);   
    GPIOPinIntEnable(KEY_PORTG, KEY_STOP);
    GPIOPinIntEnable(KEY_PORTG, KEY_RIGHT);
    GPIOPinIntEnable(KEY_PORTG, KEY_LEFT);
...
void GPIO_Port_G_ISR(void)
{
    ...
    if (ulStatus & KEY_START)                                 //  如果KEY的中断状态有效
    {
    sendflag='f';
    
     //TX_Mode('f');
    }
     if (ulStatus & KEY_STOP)                                 //  如果KEY的中断状态有效
    {
      sendflag='s';
      
  //   TX_Mode('s');
    }
    if (ulStatus & KEY_RIGHT)                                 //  如果KEY的中断状态有效
    {
      sendflag='l';
    // TX_Mode('l');
    }
     if (ulStatus & KEY_LEFT)                                 //  如果KEY的中断状态有效
    {
      sendflag='r';
    // TX_Mode('r');
    }
