...
#define FILE_NAME "data.txt" //rw file name
...
{
	file=open(FILE_NAME,O_RDWR|O_CREAT|O_TRUNC,S_IRUSR|S_IWUSR|S_IROTH|S_IWOTH|S_IRGRP);
	if(file<0)
	{
	...
	printf("high     : %10.4f \n",GPS->high);	
	printf("STATUS   : %c\n",GPS->status);	
	sprintf(buff,"#TRUCK1$DATE$%ld-%02d-%02d$TIME$%02d:%02d:%02d$Latitude$%5.4f %c$Lonitude$%5.4f %c$high$%5.4f$STATUS$%c!!",GPS->D.year,GPS->D.month,GPS->D.day,GPS->D.hour,GPS->D.minute,GPS->D.second,GPS->latitude,GPS->NS,GPS->longitude,GPS->EW,GPS->high,GPS->status);	
