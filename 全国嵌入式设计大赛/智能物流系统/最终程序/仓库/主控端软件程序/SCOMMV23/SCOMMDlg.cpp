...
BOOL CAboutDlg::OnInitDialog() 
{
	...
	m_ctrlHyperLink1.SetURL(_T("mailto:wang_sd01@126.com"));
	m_ctrlHyperLink1.SetUnderline(TRUE);
	m_ctrlHyperLink1.SetLinkCursor(AfxGetApp()->LoadCursor(IDC_CURSOR_HAND));

	m_ctrlHyperLinkWWWXFS.SetURL(_T("http://www.qrnu.edu.cn"));
...
void CSCOMMDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_RoomNum, m_RoomNum);
...
BEGIN_MESSAGE_MAP(CSCOMMDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_OpenFireAlarm, OnBUTTONOpenFireAlarm)
	ON_CBN_SELENDOK(IDC_COMBO_RoomNum, OnSelendokCOMBORoomNum)
	ON_BN_CLICKED(IDC_BUTTON_CloseFireAlarm, OnBUTTONCloseFireAlarm)
	ON_BN_CLICKED(IDC_BUTTON_OpenThiefAlarm, OnBUTTONOpenThiefAlarm)
	ON_BN_CLICKED(IDC_BUTTON_CloseThiefAlarm, OnBUTTONCloseThiefAlarm)
...
BOOL CSCOMMDlg::OnInitDialog()
{
	...
	m_RoomNum.SetCurSel(0);
	RoomNum = m_Com.GetCurSel() + 1;
...
//	m_ReceiveData+=str;
	...
	if(str == '\n')
	{
		if (currentData[0] == Address[0] && currentData[1] == Address[1] && currentData[2] == Address[2])
		{
			CTime t = CTime::GetCurrentTime();
			CString str_Time = "";
			str_Time=t.Format("%Y年%m月%d日%H时%M分%S秒\r\n");
		//	m_rFile.Write((LPCTSTR)str1,str1.GetLength());
			
			CString RoomNum ="";
			RoomNum.Format("%c%c%c",currentData[3],currentData[4],currentData[5]);
			RoomNum = "房间号："+ RoomNum +"  ";
			CString Condition = "";
			switch(currentData[6])
			{
			case '0':
				Condition+="  正常  ";break;
			case '1':
				Condition += "  一人进入  ";break;
			case '2':
				Condition += "  一人离开  ";break;
			case '3':
				Condition += "  发生火灾  ";PlayAlarmSound();break;
			case '4':
				Condition += "  发现窃贼  ";PlayAlarmSound();break;
			case '5':
				Condition += "  发现贵重物品进出 ";PlayAlarmSound();break;
			default:
				Condition += "  发送警告数据出错  ";break;
			}
			CString Status = "";
			switch(currentData[7])
			{
			case '0':
				Status += " 防火警报已关闭 ";break;
			case '1': 
				Status += "  防火警报已开启  ";break;
			default:
				Status += "  发送防火状态信息出错  ";break;
			}
			switch(currentData[8])
			{
			case '0':
				Status += "防盗警报已关闭";break;
			case '1':
				Status += "防盗警报已开启";break;
			default:
				Status += "  发送防盗状态信息出错  ";break;
			}
			CString str_CountOfPeople = "";
			int CountOfPeople = 0;
			CountOfPeople = (currentData[9] -'0')*10 + currentData[10] - '0';
			str_CountOfPeople.Format("  室内有%d个人  ",CountOfPeople);

			currentData ="";
		//	sprintf(currentData,"房间号：%s,%s,%s\n",RoomNum,Condition,Status);
			currentData.Format("%s%s,%s,%s,%s\r\n",str_Time,RoomNum,Condition,Status,str_CountOfPeople);
			m_ReceiveData+=currentData;
			UpdateData(FALSE);
			AppendSaveData(currentData);
			currentData= "";
		}
		else
		{
			if (currentData[3] == Address[0] && currentData[4] == Address[1] && currentData[5] == Address[2])
			{
				CTime t = CTime::GetCurrentTime();
				CString str_Time = "";
				str_Time=t.Format("%Y年%m月%d日%H时%M分%S秒\r\n");
				...
				CString RoomNum ="";
				RoomNum.Format("%c%c%c",currentData[0],currentData[1],currentData[2]);
				RoomNum = "房间号："+ RoomNum +"  ";
				CString Status = "";
				switch(currentData[6])
				{
				case '2':
					Status += "  防盗警报关闭命令已发出 ";break;
				case '1': 
					Status += "  防盗警报开启命令已发出 ";break;
				case '0':
					break;
				default:
					Status += "  发送防盗控制信息出错   ";break;
				}
				switch(currentData[7])
				{
				case '2':
					Status += "  防火警报关闭命令已发出 ";break;
				case '1':
					Status += "  防火警报开启命令已发出 ";break;
				case '0':
					break;
				default:
					Status += "  发送防火控制信息出错   ";break;
				}
				Status += "  正在等待回应....";
				currentData ="";
				currentData.Format("%s%s,%s\r\n",str_Time,RoomNum,Status);
				m_ReceiveData+=currentData;
				UpdateData(FALSE);
				AppendSaveData(currentData);
				currentData= "";
...
//保存显示数据
void CSCOMMDlg::AppendSaveData(CString CurrentReceiveData)
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
	...
	//文件名为Log**(年)-**(月)-**(日).txt,以下代码自动检测文件名是否存在，若不存在创建文件
	//若存在，自动在文件后面追加
	//，则后面序号自动递增如Log00.txt Log01.txt,程序自动为正要保存的文件命名为Log02.txt.
	CTime t = CTime::GetCurrentTime();
	char buf[20];  
	
	CString str1=t.Format("%Y-%m-%d");
	sprintf(buf,"Log%s.txt",str1);

	if( (access( buf, 0 )) == -1 )//若文件不存在创建文件
	{	
		if(!m_rFile.Open(buf,CFile::modeCreate|CFile::modeNoTruncate|CFile::modeReadWrite)) 
		{
			AfxMessageBox( "创建记录文件失败！");
			return;
		}
	...
	{
		m_rFile.Open(buf,CFile::modeCreate|CFile::modeNoTruncate|CFile::modeReadWrite);
	}

	
	m_rFile.SeekToEnd();

	m_rFile.Write((LPCTSTR)CurrentReceiveData,CurrentReceiveData.GetLength());
	m_rFile.Flush();
	m_rFile.Close();
}

void CSCOMMDlg::OnBUTTONOpenFireAlarm() 
{
	// TODO: Add your control notification handler code here
	if(m_Port.m_hComm==NULL)
	{
	...
	else
	{
		...
		CString MessageSend = "";
		MessageSend.Format("00%1d00001000a\r",RoomNum);
		
		if(m_ctrlHexSend.GetCheck())
		{
			char data[512];
			int len=Str2Hex(MessageSend,data);
			m_Port.WriteToPort(data,len);
			TX_count+=(long)((MessageSend.GetLength()+1)/3);
			//m_Port.WriteToPort(hexdata);	
		}
		else 
		{
			
			m_Port.WriteToPort((LPCTSTR)MessageSend);	//发送数据
			TX_count+=MessageSend.GetLength();
		}
		CString strTemp;
		strTemp.Format("TX:%d",TX_count);
		m_ctrlTXCount.SetWindowText(strTemp);
...
void CSCOMMDlg::OnSelendokCOMBORoomNum() 
{
	// TODO: Add your control notification handler code here
	RoomNum = m_RoomNum.GetCurSel() + 1; 
}

void CSCOMMDlg::OnBUTTONCloseFireAlarm() 
{
	// TODO: Add your control notification handler code here
	// TODO: Add your control notification handler code here
	if(m_Port.m_hComm==NULL)
	...
	else
	{
		...
		CString MessageSend = "";
		MessageSend.Format("00%1d00002000a\r",RoomNum);
...
void CSCOMMDlg::OnBUTTONOpenThiefAlarm() 
{
	// TODO: Add your control notification handler code here
	// TODO: Add your control notification handler code here
	if(m_Port.m_hComm==NULL)
	...
	else
	{
		...
		CString MessageSend = "";
		MessageSend.Format("00%1d00010000a\r",RoomNum);
...
void CSCOMMDlg::OnBUTTONCloseThiefAlarm() 
{
	// TODO: Add your control notification handler code here
	// TODO: Add your control notification handler code here
	if(m_Port.m_hComm==NULL)
	...
	else
	{
		...
		CString MessageSend = "";
		MessageSend.Format("00%1d00020000a\r",RoomNum);
...
void CSCOMMDlg::OnBUTTONSound() 
{
	// TODO: Add your control notification handler code here
//	sndPlaySound ("c:\music\ALARM9.wav",SND_ASYNC);
//	HINSTANCE h=AfxGetInstanceHandle(); 
//	PlaySound(MAKEINTRESOURCE(IDR_WAVE1),AfxGetApp()->m_hInstance, SND_RESOURCE | SND_ASYNC);
//	HINSTANCE h=AfxGetInstanceHandle(); 
