...
uchar TX_ADDRESS[TX_ADR_WIDTH] = {0x34,0x43,0x10,0x10,0x00};
...
char str_send[12];
extern char SourceAddress[3];
extern char TargetAddress[3];
...
void sendData(char status)
{

 // switch(status)
//  {
//  case '0':
    sprintf(str_send,"%3s%3s%1d%1d%1d%c%ca",TargetAddress,SourceAddress,status,modeforg,modebody,count/10+'0',count%10+'0');
    
    sendCodeData(str_send);
    SysCtlDelay(200*(TheSysClock/3000));
    RX_Mode();
    SysCtlDelay(200*(TheSysClock/3000));   
