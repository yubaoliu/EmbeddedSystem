...
void show_gps(GPS_INFO *GPS)
{
	...
	printf("high     : %10.4f \n",GPS->high);	
	printf("STATUS   : %c\n",GPS->status);	
	sprintf(buff,"#DATE: %ld-%02d-%02d \nTIME: %02d:%02d:%02d \nLatitude:%10.4f %c\nLonitude:%10.4f %c\nhigh:%10.4f \nSTATUS:%c\n!!",GPS->D.year,GPS->D.month,GPS->D.day,GPS->D.hour,GPS->D.minute,GPS->D.second,GPS->latitude,GPS->NS,GPS->longitude,GPS->EW,GPS->high,GPS->status);	
